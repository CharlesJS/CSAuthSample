//
//  CFNumber+CSAuthSampleCommon.swift
//  CSAuthSampleCommon
//
...
extension CFBoolean: XPCConvertible {
    public static func fromXPCObject(_ xpcObject: xpc_object_t) -> XPCConvertible? {
        xpc_bool_get_value(xpcObject) ? kCFBooleanTrue : kCFBooleanFalse
    }
...
extension CFNumber: XPCConvertible {
    public static func fromXPCObject(_ xpcObject: xpc_object_t) -> XPCConvertible? {
        ...
        case XPC_TYPE_INT64:
            ...
            return CFNumberCreate(kCFAllocatorDefault, .sInt64Type, &int)
        case XPC_TYPE_UINT64:
            var uint = xpc_int64_get_value(xpcObject)

            return CFNumberCreate(kCFAllocatorDefault, .sInt64Type, &uint)
        case XPC_TYPE_DOUBLE:
            var double = xpc_double_get_value(xpcObject)

            return CFNumberCreate(kCFAllocatorDefault, .doubleType, &double)
        default:
            return nil
        }
    }
