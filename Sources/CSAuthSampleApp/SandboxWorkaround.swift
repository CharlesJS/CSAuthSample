...
public class SandboxWorkaround {
    /// Create an instance of `SandboxWorkaround`.
    ///
    /// Be sure to call `stop()` once you receive a reply from the helper tool.
    public init() {
        self.queue.async(execute: self.activateIfReady)
    }

    /// Call this within the reply block after sending a message to your helper tool.
    public func stop() {
    ...
    private func activateIfReady() {
        ...
        if let securityAgent = NSRunningApplication.runningApplications(
            withBundleIdentifier: agentID
        )
        .last {
